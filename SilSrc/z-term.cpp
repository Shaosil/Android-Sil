...
static errr term_win_copy(term_win *s, term_win *f, int w, int h)
{
	...
	{
		byte *f_aa = f->a[y];
		signed char *f_cc = f->c[y];

		byte *s_aa = s->a[y];
		signed char *s_cc = s->c[y];

		byte *f_taa = f->ta[y];
		signed char *f_tcc = f->tc[y];

		byte *s_taa = s->ta[y];
		signed char *s_tcc = s->tc[y];
 ...
 */
static errr Term_text_hack(int x, int y, int n, byte a, const signed char *cp)
{
	/* Compiler silliness */
 ...
 */
static errr Term_pict_hack(int x, int y, int n, const byte *ap, const signed char *cp, const byte *tap, const signed char *tcp)
{
	/* Compiler silliness */
...
void Term_queue_char(int x, int y, byte a, char c, byte ta, char tc)
{
	...
	signed char *scr_cc = Term->scr->c[y];

	byte oa = scr_aa[x];
	signed char oc = scr_cc[x];

	byte *scr_taa = Term->scr->ta[y];
	signed char *scr_tcc = Term->scr->tc[y];

	byte ota = scr_taa[x];
	signed char otc = scr_tcc[x];
...
static void Term_fresh_row_pict(int y, int x1, int x2)
{
	...
	signed char *old_cc = Term->old->c[y];

	byte *scr_aa = Term->scr->a[y];
	signed char *scr_cc = Term->scr->c[y];

	byte *old_taa = Term->old->ta[y];
	signed char *old_tcc = Term->old->tc[y];
...
errr Term_fresh(void)
{
	...
	if (Term->total_erase)
	{
		...
		{
			byte *aa = old->a[y];
			signed char *cc = old->c[y];
			byte *taa = old->ta[y];
			signed char *tcc = old->tc[y];
	...
	if (Term->soft_cursor)
	{
		...
		if (!old->cu && old->cv)
		{
			...
			signed char *old_cc = old->c[ty];

			byte oa = old_aa[tx];
			signed char oc = old_cc[tx];

			byte *old_taa = old->ta[ty];
			signed char *old_tcc = old->tc[ty];

			byte ota = old_taa[tx];
			signed char otc = old_tcc[tx];
