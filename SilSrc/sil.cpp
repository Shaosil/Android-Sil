...
static errr Term_xtra_and(int n, int v)
{
	...
	switch (n)
	{
		case TERM_XTRA_EVENT:
		{
			...
			if (key < 0) {
				const int should_save = (turn_save != turn && turn > 1);
 ...
 */
static errr Term_text_and(int x, int y, int n, byte a, const signed char *cp)
{
	move(y, x);

	const byte flag = a / MAX_COLORS;

	a %= MAX_COLORS;

	const int r = angband_color_table[a][1] << 16;
	const int g = angband_color_table[a][2] << 8;
	const int b = angband_color_table[a][3];
	const int clr = 0xFF000000 | r | g | b;

	addnstr(clr, n, cp, flag);
 ...
 */
static errr Term_pict_and(int x, int y, int n, const byte *ap, const signed char *cp,
						  const byte *tap, const signed char *tcp)
{
	//LOGD("Term_pict_and");
...
void sil_process_argv(int i, const char* argv) {
	switch(i){
	case 0: //files path
