...
public class GameActivity extends Activity {
	...
	private AngbandKeyboardView kbView = null;
	private boolean kbOverlappingAndVisible = false;
	...
	protected void rebuildViews() {
		synchronized (state.progress_lock) {
			...
			if (kb) {
				kbView = (AngbandKeyboardView)new AngbandKeyboard(this).virtualKeyboardView;
				kbView.setLayoutParams(new LayoutParams(LayoutParams.MATCH_PARENT, LayoutParams.WRAP_CONTENT));
			}
			...
			if (Preferences.getKeyboardOverlap()) {
				...
				if (kb) {
					RelativeLayout.LayoutParams params = new RelativeLayout.LayoutParams(kbView.getLayoutParams());
					params.addRule(RelativeLayout.ALIGN_PARENT_BOTTOM);
					kbView.setLayoutParams(params);
					screenLayout.addView(kbView);
				...
				if (kb) enclosure.addView(kbView);
	...
	public int getKeyboardOverlapHeight() {
		return kbOverlappingAndVisible ? kbView.getHeight() : 0;
	}
}